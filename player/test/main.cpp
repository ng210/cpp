...
class PlayerTest : public Test {
    ...
    static HANDLER_FUNC(setter, Value);
...
int PlayerTest::setter(void* obj, Value value, void* args) {
    var input = (Input*)obj;
...
void PlayerTest::testInput() {
    Value value;
    Input input(&value);
    input.Input::initialize(10, 110, 6);
    input.set(22);
    ...
    input.set.add(&input, PlayerTest::setter, this);
    input.set(44);
...
void PlayerTest::testInputF() {
    Value value;
    InputF input(&value);
    input.initialize(-10.0f, 10.0f, 0.25f);
    input.set(0.7f);
...
void PlayerTest::testCreateConsDeviceExt() {
    ...
    assert("Should bind to device values",
        consDevice->getValue(0) == consDeviceExt->getInput(0)->value &&
        consDevice->getValue(1) == consDeviceExt->getInput(1)->value &&
        consDevice->getValue(2) == consDeviceExt->getInput(2)->value);
    DEL_(consDeviceExt);
    DEL_(playerExt);
    DEL_(playerDevice);
    Player::cleanUp();
...
Stream* PlayerTest::createBinaryData() {
    ...
    // 2nd adapter: ConsAdapter
    {
        ...
        adapterList->writeByte(ConsDevices::DeviceCons);    // type
        adapterList->writeByte(-1);                         // preset id

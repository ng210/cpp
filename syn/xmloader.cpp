...
XmLoader::~XmLoader(void) {
	ARRAY_FOREACH(&patterns_, for (int j = 0; j < channelCount_; j++) DEL_(((XMPATTERN*)value)->channels[j]); FREE(value););
	DEL_(xmEffectMap_);
}
...
XMPATTERN* XmLoader::readPattern(XMFILEPATTERN* ptr) {
	int allocSize = sizeof(XMPATTERN) + sizeof(XMNOTE)*ptr->rowCount*channelCount_;
	XMPATTERN* pattern = (XMPATTERN*)MALLOC(UINT8, allocSize);
	memset(pattern, 0, allocSize);

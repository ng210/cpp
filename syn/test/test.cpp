...
static void registerPlugins() {
	...
	for (UINT32 i=0; i<files->length(); i++) {
		...
		if (getAdapterInfos != NULL) {
			...
			ARRAY_FOREACH(adapterInfos, Asu::registerAdapter((ADAPTER_INFO*)value););
		}
	}
}
...
static Asu* createSong(const char* path) {
	Asu* asu = new Asu();
	const ADAPTER_INFO* adapterInfo = Asu::getAdapterInfo(0x4e5953);
	if (adapterInfo != NULL) {
		UINT8* data = NULL;
...
int _main(NS_FW_BASE::Map* args) {
	...
	// *************************************
	asu = createSong("C:\\code\\git\\cpp\\syn\\data\\1asu.xm");
